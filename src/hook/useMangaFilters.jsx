...
const useMangaFilters = (location, currentPage, setMangas, setTotalMangas) => {
  ...
  const fetchMangas = async (page) => {
    ...
    try {
      ...
      const resp = await axios.get(`https://cors-anywhere.herokuapp.com/${baseUrl}/manga`, { params: filters });
  ...
  const fetchTagUUIDs = async (tagNames) => {
    try {
      const response = await axios.get('https://cors-anywhere.herokuapp.com/' + `${baseUrl}/tag`);
